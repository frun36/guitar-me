...
volatile uint16_t DAC_Buf[BUF_SIZE] = {};

volatile int ADC_FirstHalf = 0;
volatile int ADC_SecondHalf = 0;
...
int main(void) {
    ...
    while (1) {
        ...
        if (ADC_SecondHalf) {
            ADC_SecondHalf = 0;
            memcpy(
                (void*)(DAC_Buf + BUF_SIZE_HALF),
                (const void*)(ADC_Buf + BUF_SIZE_HALF),
                BUF_SIZE_HALF * sizeof(uint16_t)
            );
        }
